...
var BILI_AI = (() => {
  ...
  /**
   * 自然句可能跨越多个原始字幕条目。翻译时把这些真实来源片段拆成稳定的
   * 内部单元，避免模型只译其中一段却把整个自然句标记为成功。
   */
  function buildTranslationUnits(segments) {
    const units = [];
    for (const segment of Array.isArray(segments) ? segments : []) {
      const parts = Array.isArray(segment?.sourceParts) && segment.sourceParts.length
        ? segment.sourceParts
        : [{
            entryIndex: segment?.sourceEntryIndexes?.[0] ?? 0,
            start: segment?.start,
            text: segment?.text,
          }];
      parts.forEach((part, partIndex) => {
        units.push({
          ...
          start: Number(part?.start ?? segment.start) || 0,
          sourceEntryIndexes: [part?.entryIndex],
  ...
  /** 只有一个自然句的每个内部单元都有译文时，才生成对 UI 可见的整句译文。 */
  function composeTranslatedSegments(units, translatedParts) {
    ...
    for (const [parentId, parts] of byParent) {
      ...
      if (ordered.some((part) => !translatedParts?.[part.id])) continue;
      translated[parentId] = ordered
        .map((part) => String(translatedParts[part.id]).trim())
        .filter(Boolean)
        .join(" ");
    }
    return translated;
