...
var BILI_TRANSCRIPT_SERVICE = (() => {
  ...
  function createTranscriptService({
     ...
     * 重建句子。v3 → v4 只增加翻译来源片段，句子 id 没变，因此可保留能
     * 对回当前 id 的顺句稿；旧译文无法证明覆盖完整，必须失效。更老版本的
     * 句子边界不同，顺句与译文都不能硬套。其余学习资料原样保留。
     */
    function normalizeSegmentSchema(payload) {
      ...
      const segments = TRANSCRIPT.groupTranscriptEntries(payload.transcript);
      const segmentIds = new Set(segments.map((segment) => segment.id));
      const polished = Number(payload.segmentSchemaVersion) === 3
        ? Object.fromEntries(
            Object.entries(payload.polished || {}).filter(([id]) => segmentIds.has(id)),
          )
        : {};
      return {
        migrated: true,
