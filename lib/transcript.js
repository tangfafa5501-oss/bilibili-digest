...
var BILI_TRANSCRIPT = (() => {
  ...
  // 错贴到一句新字幕上。
  const SEGMENT_SCHEMA_VERSION = 6;
  ...
  function cueIdForEntry(entry, entryIndex) {
    const start = Number(entry?.start) || 0;
    const duration = Math.max(0, Number(entry?.duration) || 0);
    return `cue-${entryIndex}-${Math.round(start * 1000)}-${Math.round(duration * 1000)}`;
  }

  function canonicalizeEntries(entries) {
    return (Array.isArray(entries) ? entries : []).map((entry, entryIndex) => ({
      ...entry,
      text: normalizeCaptionText(entry?.text),
      start: Math.max(0, Number(entry?.start) || 0),
      duration: Math.max(0, Number(entry?.duration) || 0),
      cueId: cueIdForEntry(entry, entryIndex),
    }));
  }

  function makeSegment({ text, start, id, sourceEntryIndexes, sourceParts, fallback }) {
    const normalized = normalizeCaptionText(text);
    const normalizedParts = (Array.isArray(sourceParts) ? sourceParts : []).map((part) => ({
      entryIndex: part.entryIndex,
      cueId: part.cueId,
      start: part.start,
      duration: part.duration,
      text: normalizeCaptionText(part.text),
    }));
    return {
      id,
      start,
      ...
      texts: [normalized],
      sourceEntryIndexes,
      sourceCueIds: normalizedParts.map((part) => part.cueId),
   ...
   * - 原始 cue 是翻译与时间轴的不可分割单位；自然句只在 cue 边界聚合，
   *   不把一个 cue 拆成多个无法独立翻译的伪 cue；
   * - 文件末尾没有可靠句末标点的内容按原始条目回退，保证仍可精确定位。
   *
   * 无标点 run 超过可靠窗口时，整段按原始条目回退；不能因为很晚才出现
   * 一个句号，就把几十秒甚至几分钟的字幕冒充成一个自然句。
   */
  function groupTranscriptEntries(entries, limits) {
    if (!Array.isArray(entries) || entries.length === 0) return [];
    const effectiveLimits = { ...limitsForEntries(entries), ...(limits || {}) };
    const grouped = [];
    let pending = null;

    const appendPending = ({ text, start, duration, end, entryIndex, cueId }) => {
      if (!pending) {
        pending = {
          start,
          end,
      ...
      pending.byEntry.set(entryIndex, { cueId, start, duration, text });
    };
    ...
    const flushSentence = () => {
      ...
      grouped.push(
        makeSegment({
          ...
          start: pending.start,
          id: `segment-s${pending.firstEntryIndex}-0-${Math.round(pending.start * 1000)}`,
          sourceEntryIndexes: [...pending.byEntry.keys()],
          sourceParts: [...pending.byEntry].map(([entryIndex, entry]) => ({
            entryIndex,
            cueId: entry.cueId,
            start: entry.start,
    ...
    const pendingExceeded = () => {
      if (!pending) return false;
      const chars = normalizeCaptionText(pending.parts.join(" ")).length;
      return (
        pending.end - pending.start > effectiveLimits.maxSeconds ||
        chars > effectiveLimits.maxChars ||
        pending.byEntry.size > effectiveLimits.maxEntries
      );
    };
    ...
    entries.forEach((entry, entryIndex) => {
      ...
      const duration = Number.isFinite(Number(entry?.duration))
        ? Math.max(0, Number(entry.duration))
        : 0;
      ...
      const ended = SENTENCE_END_PATTERN.test(text);
      if (pendingExceeded()) flushFallback();
      else if (ended) flushSentence();
    });
