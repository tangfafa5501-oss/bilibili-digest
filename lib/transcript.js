...
var BILI_TRANSCRIPT = (() => {
  ...
  // 错贴到一句新字幕上。
  const SEGMENT_SCHEMA_VERSION = 4;
  ...
  function makeSegment({ text, start, id, sourceEntryIndexes, sourceParts, fallback }) {
    ...
    return {
      ...
      texts: [normalized],
      sourceEntryIndexes,
      sourceParts: (Array.isArray(sourceParts) ? sourceParts : []).map((part) => ({
  ...
  function groupTranscriptEntries(entries, limits) {
    ...
    const appendPending = ({ text, start, end, entryIndex, partIndex }) => {
      if (!pending) {
        pending = {
          start,
          end,
      ...
      for (const [entryIndex, entry] of pending.byEntry) {
        grouped.push(
          makeSegment({
            ...
            sourceParts: [{ entryIndex, start: entry.start, text: entry.parts.join(" ") }],
            fallback: true,
          }),
        );
      }
    ...
    entries.forEach((entry, entryIndex) => {
      ...
      sentenceParts.forEach((sentencePart, partIndex) => {
        ...
        if (!cleanPart) return;
        appendPending({ text: cleanPart, start, end, entryIndex, partIndex });
        const ended = SENTENCE_END_PATTERN.test(cleanPart);
